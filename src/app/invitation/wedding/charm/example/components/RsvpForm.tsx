...
import { RsvpSection } from "@/types/data";
...
export default function RsvpForm({ rsvp }: { rsvp: RsvpSection }) {
  ...
  const addGuest = () =>
    setGuests([...guests, { name: "", age: "", diet: "" }]);
  const removeGuest = (index: number) =>
    setGuests(guests.filter((_, i) => i !== index));

  // --- ✅ Перевірка, чи введено хоч одне ім’я
  const hasAtLeastOneName = guests.some((guest) => guest.name.trim() !== "");
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p className="text-[var(--charm-dark)]/80">
            Ми з нетерпінням чекаємо зустрічі з вами на нашому святі!
          </p>
        </motion.div>
      </section>
    );
  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto">
        <motion.h2
          className="text-center text-2xl md:text-4xl mb-6 font-light"
          variants={fadeUp}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true }}
        ...
        <p className="text-center text-[var(--charm-dark)]/80 mb-8">
          {rsvp.text}
        </p>
        ...
        >
          ...
          <div>
            <label className="block mb-2 text-lg">
              {rsvp.form.fields.attendance.label}
            </label>
            <div className="flex gap-6">
              {rsvp.form.fields.attendance.options?.map((option, i) => (
                <label
                  key={i}
                ...
                >
                  <input
                    type="radio"
                    name={`entry.${rsvp.form.fields.attendance.googleEntryId}`}
                    value={option}
                    className="accent-[var(--charm-accent)]"
                    required
          ...
          {attending === "Так" && (
            <div className="space-y-8">
              {guests.map((guest, index) => (
                <div
                  key={index}
                  className="border border-[var(--charm-accent)] rounded-md p-4 relative bg-[var(--charm-cream)]"
                >
                  {guests.length > 1 && (
                    <button
                      type="button"
                      onClick={() => removeGuest(index)}
                      className="absolute right-3 top-2 text-sm cursor-pointer text-[var(--charm-dark)]/60 hover:text-red-500"
                    >
                      ×
                    </button>
                  )}
                  ...
                  <label className="block mb-2">
                    {rsvp.form.fields.guests.fields.name.label}
                  </label>
                  <input
                    name={`entry.${rsvp.form.fields.guests.fields.name.googleEntryId}-${index}`}
                    type="text"
                    required
                    ...
                    className="w-full border p-3 rounded-sm bg-[var(--charm-light)] focus:ring-2 focus:ring-[var(--charm-accent)] outline-none"
                  />

                  {/* Вік */}
                  <label className="block mt-4 mb-2">
                    {rsvp.form.fields.guests.fields.age.label}
                  </label>
                  <select
                    name={`entry.${rsvp.form.fields.guests.fields.age.googleEntryId}-${index}`}
                    value={guest.age}
                    onChange={(e) => {
                      const newGuests = [...guests];
                    ...
                    className="w-full border p-3 rounded-sm bg-[var(--charm-light)]"
                  >
                    <option value="">Оберіть</option>
                    {rsvp.form.fields.guests.fields.age.options?.map(
                      (option, i) => (
                  ...
                  {/* Дієтичні побажання */}
                  <label className="block mt-4 mb-2">
                    {rsvp.form.fields.guests.fields.diet.label}
                  </label>
                  <textarea
                    name={`entry.${rsvp.form.fields.guests.fields.diet.googleEntryId}-${index}`}
                    value={guest.diet}
                    onChange={(e) => {
                      const newGuests = [...guests];
                      newGuests[index].diet = e.target.value;
                      setGuests(newGuests);
                    }}
                    className="w-full border p-3 rounded-sm bg-[var(--charm-light)] h-20 focus:ring-2 focus:ring-[var(--charm-accent)] outline-none"
                  />
                </div>
              ))}
              ...
              <div className="text-center">
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  onClick={addGuest}
                  className="border border-[var(--charm-dark)] cursor-pointer px-6 py-2 rounded-sm hover:bg-[var(--charm-dark)] hover:text-[var(--charm-light)] transition-all duration-300"
                >
                  + Додати ще гостя
                </motion.button>
              </div>
          ...
          {/* === Кнопка Submit (зʼявляється лише якщо заповнено хоч одне ім’я) === */}
          <AnimatePresence>
            {attending === "Так" && hasAtLeastOneName && (
              <motion.div
                key="submit-button"
              ...
              >
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  type="submit"
                  className="bg-[var(--charm-dark)] text-[var(--charm-light)] cursor-pointer px-8 py-3 rounded-sm hover:bg-[var(--charm-accent)] hover:text-[var(--charm-dark)] transition-all duration-300"
                >
                  {rsvp.form.submitText}
                </motion.button>
              </motion.div>
            )}
          </AnimatePresence>
